 ...
 * counts slice. It parses a `counts` reply ({"sources":{…}}) and holds it
 * in the `view` field for <SourceCounts/>; it never touches the top or
 * accumulated slices.
 */
...
describe( 'source-counts:view', () => {
	...
	test( 'parses a counts reply into the sources slice and publishes it', () => {
		...
		expect( v.view ).toEqual( { sources: { github: 2 } } );
	} );
